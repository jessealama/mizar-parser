...
(define-constant +information-message+
    (with-html-output-to-string (dummy)
      ...
      (:body
       ...
       (:p "The intention behind this site is to facilitate programmatic access to Mizar parsing services.")
       (:p "At the moment no HTML-driven interface to the parser services is provided; you are now looking at the only HTML page that this service emits, which is entirely informational.")
  ...
  (let* ((method-name (symbol-name method))
    ...
    (cond ((string= method-name-lc "post")
	  ...
	  ((string= method-name-lc "options")
	   (call-next-method))
	  ((string= method-name-lc "get")
	   (call-next-method))
	  (t
	   (setf (header-out "Allow") "GET, POST, HEAD, OPTIONS")
...
(defmacro handle-file-get (path mime-type)
